<template>
  <div class="create-picking-container">
    <el-card class="picking-card" shadow="never">
      <!-- ==================== 顶部标题区 ==================== -->
      <template #header>
        <div class="card-header">
          <h2 class="card-title">创建拣货订单</h2>
            ...
            <el-col :xs="24" :sm="12" :md="8">
              <el-form-item label="拣货类型">
                <el-select
                  ...
                  style="width: 100%"
                >
                  <el-option label="订单拣货" value="订单拣货" />
                  <el-option label="批量拣货" value="批量拣货" />
                  <el-option label="波次拣货" value="波次拣货" />
                  <el-option label="补货拣货" value="补货拣货" />
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :xs="24" :sm="12" :md="8">
      ...
      <div class="section">
        ...
        <div class="material-search-area">
          ...
          />
          <el-button type="primary" size="large" :icon="Search" @click="handleSearchEnter">
            添加物料
          </el-button>
        </div>
        <p class="search-hint">支持扫描枪/键盘输入物料编码后直接回车；也可点击上方"导入 Excel 明细"批量添加。</p>
      </div>
        ...
        >
          ...
          <template #empty>
            <div class="empty-state">
              ...
              <p class="empty-hint">请通过上方输入框输入物料编码回车添加，或点击"导入 Excel 明细"批量导入</p>
            </div>
          </template>
...
import { createPickingOrder, type MaterialRequest } from '@/api/outbound'
...
/** 回车/点击搜索 — 调用 getMaterialInfo 接口查询并添加到表格 */
...
/** Excel 模板表头（与 PickingOrderItem 字段一致） */
...
const handleSubmit = async () => {
  ...
  // 构建 materialRequests 数组（严格按 PickingListRequest 定义）
  // 后端 MaterialRequest 使用 ean 查询库存并自动按库位分配
  const materialRequests: MaterialRequest[] = tableData.value.map(row => ({
    ean: row.ean || row.material,
  ...
  // 组装请求体（严格按 POST /inventory/pickingList API 定义）
  const requestBody = {
    materialRequests,
  ...
  try {
    const response: any = await createPickingOrder(requestBody)

    // 成功 — code: 200（request 拦截器已解包 AxiosResponse，返回 Result<T> 结构）
    if (response.code === 200) {
      // 后端生成订单号，创建成功后从响应中获取
      const createdOrderNo = response.data?.orderNo || orderForm.value.orderNo
      ElMessage.success({
        message: `拣货订单创建成功！订单号：${createdOrderNo}`,
        duration: 3000
      })

      // 跳转到按订单拣货页
      setTimeout(() => {
        router.push('/picking/order-picking')
      }, 1200)
    } else {
      // 非 200 的业务错误
      globalErrorMsg.value = response.message || '拣货订单创建失败'
    }
  } catch (error: any) {
    // 网络异常或拦截器已处理的错误
    const errMsg =
